...
> Node.js 빌트인만 사용 (npm 의존성 0). 마지막 갱신: 2026-06-13.
> **운영 호스트: AWS EC2(서울 ap-northeast-2, 15.164.129.5) 24시간 상시** — 2026-06-13 로컬 VM에서 이전.
> 접속 `ssh -i autotrade-key.pem ubuntu@15.164.129.5`, 경로 `/home/ubuntu/KoreanInvestment`. 자세한 건 DEPLOY.md.
...
auto-trader.js    자동매매 엔진. **추세 돌파 전략**(신고가 돌파+거래량, 2026-06 전면 개편 — 구 골든크로스+RSI 폐기),
                  KOSPI 레짐 필터(069500 프록시), 부분익절+샹들리에 트레일링, ATR 손절, 일일 손실 한도,
...
tests/            engine(75)·realtime(23)·fallback(23)·journal(26)·auth(18) = 165개. node tests/xxx-test.js
```

## 2. 실행 (운영=EC2, 아래는 새 호스트 구축 시)

운영은 EC2에서 systemd `autotrade.service`로 상시 가동(`sudo systemctl status autotrade`). 새 PC/서버 구축 시:
```bash
# Node 22+ 설치 후
...
# 테스트 (165개 전부 통과해야 정상)
node tests/engine-test.js && node tests/realtime-test.js && node tests/fallback-test.js && node tests/journal-test.js && node tests/auth-test.js
```
⚠️ **이중 봇 금지**: 같은 KIS 계좌에 자동매매 서버를 둘 이상 띄우지 말 것(주문 충돌). 호스트 이전 시
옛 호스트의 `autotrade.service`는 `sudo systemctl disable --now autotrade`로 반드시 끌 것.
...
14. **AWS EC2 이전 (06-13)**: 로컬 VM은 PC 끄면 같이 꺼져 06-11·12 장에 자동매매가 끊김(손절 미작동 포함).
    → **EC2(서울, t3.micro, Ubuntu)로 이전해 24h 상시 운영**. 미push 3커밋 GitHub 반영 후 EC2 pull,
...
16. **매매 전략 전면 개편 — 추세 돌파형 (06-16)**: 모의 -500만 손실 원인 진단(진입 늦음·RSI70 조기 매도로
    실질 손익비 붕괴·시장 필터 부재·과매매 비용) → 구 전략(골든크로스+RSI) 폐기.
    ⓐ **진입**: N(20)일 신고가 '종가' 돌파 + 거래량 평균 1.5배 확인 + 종목 장기MA 비하락.
    ⓑ **시장 레짐 필터**: KODEX200(069500) 일봉을 지수 프록시로, 지수<20MA면 신규매수 중단(기존 차트 dep 재사용).
    ⓒ **청산**: 고정 +2R 익절·RSI 과매수 매도 **폐지** → +1R 절반 익절(+손절 본전) 후 샹들리에 트레일링,
      10일 저가 이탈 시 추세 종료 매도. 손절 판정이 부분익절보다 항상 우선(스탑 걸리면 전량 시장가).
    ⓓ takeProfitR 폐기(설정·AI 비서 화이트리스트 trailAfterR로 교체), 프론트 전략/백테스트 탭 신전략 교체.
    ⓔ **tools/backtest.js** 신설 — 신구 전략 비교(네이버 실데이터/`--synthetic`). 합성 검증: 구 319건 -0.006R/건(PF 0.98)
      vs 신 +0.62R/건(PF 4.2). ⚠️ 실데이터 백테스트는 EC2에서 `node tools/backtest.js` 실행. 테스트 165→**232**.
