...
// KIS 실시간 레코드는 작다(수백 바이트). 평문 ws:// 라 온패스/손상 프레임이 거대한 길이를
// 선언하면 무한 버퍼 누적(OOM)·거대 slice(RangeError)로 이어질 수 있어 상한을 둔다.
const MAX_FRAME = 1 << 20;      // 단일 프레임/재조립 메시지 최대 1 MiB
const MAX_BUF   = 2 << 20;      // 수신 누적 버퍼 최대 2 MiB
...
class MiniWS {
  ...
  connect() {
    ...
    this.sock.on('data', chunk => {
     try {
      ...
      if (this.buf.length > MAX_BUF) { this._fail(new Error('WS 수신버퍼 과대(' + this.buf.length + ')')); return; }
      if (!handshook) {
      ...
      this._drain();
     } catch (e) { this._fail(e); } // 파서 예외는 크래시 대신 재연결로
    });
  ...
  _drain() {
    while (true) {
      ...
      // 과대 프레임 차단(손상/공격) — 버퍼 영원 누적·거대 slice 방지, 재연결로 복구
      if (!(len >= 0) || len > MAX_FRAME) { this._fail(new Error('WS 프레임 과대(' + len + ')')); return; }
      ...
      if (opcode === 0x1 || opcode === 0x2 || opcode === 0x0) {
        if (opcode !== 0x0) this._frag = { op: opcode, parts: [payload], size: payload.length };
        else if (this._frag) { this._frag.parts.push(payload); this._frag.size += payload.length; }
        // 재조립 메시지 누적 상한 — 무한 continuation 프레임으로 메모리 소진 방지
        if (this._frag && this._frag.size > MAX_FRAME) { this._fail(new Error('WS 메시지 과대(' + this._frag.size + ')')); return; }
        if (fin && this._frag) {
